...
const zkgapi = require('@hyperoracle/zkgraph-api');
...
const createZkgraph = require('create-zkgraph');
...
const to__default = /*#__PURE__*/_interopDefaultCompat(to);
const FormData__default = /*#__PURE__*/_interopDefaultCompat(FormData);
const axios__default = /*#__PURE__*/_interopDefaultCompat(axios);
const zkgapi__namespace = /*#__PURE__*/_interopNamespaceCompat(zkgapi);
const colors__default = /*#__PURE__*/_interopDefaultCompat(colors);
const ethres__default = /*#__PURE__*/_interopDefaultCompat(ethres);
...
const api__default = /*#__PURE__*/_interopDefaultCompat(api);
...
function createLogger(level = "info", prefix = "[zkGraph]") {
  ...
  function output(type, msg) {
    if (thresh >= LogLevels[type]) {
      ...
      const format = () => {
        const tag = type === "info" ? colors__default.cyan(colors__default.bold(prefix)) : type === "warn" ? colors__default.yellow(colors__default.bold(prefix)) : colors__default.red(colors__default.bold(prefix));
        return `${colors__default.dim(timeFormatter.format( new Date()))} ${tag} ${msg}`;
      };
...
async function codegen(mappingRoot, filename, content) {
  return new Promise((resolve, reject) => {
    try {
      const filepath = path__default.join(mappingRoot, filename);
...
const sdk = api__default("@pinata-cloud/v1.0#12ai2blmsggcsb");
async function checkPinataAuthentication(pinataJWT) {
  sdk.auth(pinataJWT);
  const [err, res] = await to__default(sdk.getDataTestauthentication());
  if (err)
    return false;
...
function checkExecExist(command) {
  try {
    node_child_process.execSync(command, { stdio: "ignore" });
    return true;
  } catch (error) {
    return false;
  }
...
async function compileLocal(options) {
  ...
  const yaml = zkgapi__namespace.ZkGraphYaml.fromYamlPath(yamlPath);
  if (!yaml) {
    logger.error("[-] ERROR: Failed to get yaml");
    return false;
  }
  const dsp = zkgapi__namespace.dspHub.getDSPByYaml(yaml, { isLocal: local });
  const libDSPName = dsp.getLibDSPName();
  const mappingFileName = yaml.mapping.file;
  const handleFuncName = yaml.mapping.handler;
  const dspKey = zkgapi__namespace.dspHub.toHubKeyByYaml(yaml, { isLocal: local });
  const srcDirPath = path__default.join(mappingPath, "..");
  ...
  const [compileErr] = await to__default(ascCompile(path__default.join(srcDirPath, entryFilePath), wasmPath, watPath));
  if (compileErr) {
...
async function compileServer(options) {
  ...
  const tmpWasmPath = path__default.join(path__default.dirname(wasmPath), "/temp/inner_pre_pre.wasm");
  ...
  data.append("wasmFile", fs__default.createReadStream(tmpWasmPath));
  data.append("yamlFile", fs__default.createReadStream(yamlPath));
  const requestConfig = {
    method: "post",
  ...
  const [requestErr, response] = await to__default(axios__default.request(requestConfig));
  if (requestErr) {
  ...
  createOnNonexist(wasmPath);
  fs__default.writeFileSync(wasmPath, fromHexString(wasmModuleHex));
  createOnNonexist(watPath);
  fs__default.writeFileSync(watPath, wasmWat);
  return true;
}
...
function logCompileResult(wasmPath, watPath) {
  const compiledFileContent = fs__default.readFileSync(watPath, "utf-8");
  ...
  logger.info(`[+] Output written to \`${path__default.dirname(wasmPath)}\` folder.`);
...
async function exec(options) {
  ...
  const zkgraphYaml = zkgapi__namespace.ZkGraphYaml.fromYamlPath(yamlPath);
  if (!zkgraphYaml) {
    logger.error("[-] ERROR: Failed to get yaml");
    return;
  }
  const dsp = zkgapi__namespace.dspHub.getDSPByYaml(zkgraphYaml, { isLocal: local });
  if (!dsp) {
    logger.error("[-] ERROR: Failed to get DSP");
    return;
  }
  const [generateErr, realParams] = await to__default(generateDspHubParams(dsp, params, "exec"));
  if (generateErr) {
  ...
  const provider = new ethres__default.providers.JsonRpcProvider(jsonRpcUrl);
  const wasm = fs__default.readFileSync(wasmPath);
  ...
  const state = await zkgapi__namespace.execute(
    zkgraphExecutable,
    execParams,
    local,
    true
...
axiosRetry__default(axios__default, {
  retries: 3,
...
function getDispatcher(userPrivateKey) {
  const provider = new ethres.ethers.providers.JsonRpcProvider(TdConfig.providerUrl);
  const signer = new ethres.ethers.Wallet(userPrivateKey, provider);
  const feeInWei = ethres.ethers.utils.parseEther(TdConfig.fee);
  const dispatcher = new zkgapi.TaskDispatch(TdConfig.queryrApi, TdConfig.contract, feeInWei, provider, signer);
  return dispatcher;
}
async function uploadWasmToTd(wasmPath) {
  const data = new FormData__default();
  data.append("file", fs__default.createReadStream(wasmPath));
  const requestConfig = {
    method: "post",
    maxBodyLength: Infinity,
  ...
  const response = await axios__default.request(requestConfig).catch((error) => {
    throw error;
  });
...
async function setup(options) {
  ...
  const deatails = await zkgapi.zkwasm_imagedetails(zkWasmProviderUrl, md5);
  ...
  const result = await zkgapi.waitSetup(zkWasmProviderUrl, taskId, true);
  return result;
}
...
async function prove(options) {
  ...
  const [generateErr, realParams] = await to__default(generateDspHubParams(dsp, params, "prove"));
  if (generateErr) {
    console.log(generateErr.message);
    return;
  }
  ...
  const [validateErr] = await to__default(validateProvider(provider));
  if (validateErr) {
  ...
  const [privateInputStr, publicInputStr] = await zkgapi__namespace.proveInputGen(
    zkgraphExecutable,
    proveParams,
    local,
    true
...
async function testMode(wasmUint8Array, privateInputStr, publicInputStr) {
  ...
  const [mockErr, mockSuccess] = await to__default(zkgapi__namespace.proveMock(
    zkgraphExecutable,
    privateInputStr,
    publicInputStr
  ));
  ...
  if (mockSuccess) {
    ...
    if (zkgapi__namespace.hasDebugOnlyFunc)
...
async function proveMode(userPrivateKey, md5, privateInputStr, publicInputStr, zkWasmProviderUrl, outputProofFilePath) {
  ...
  const [queryTaskErr, data] = await to__default(dispatcher.queryTask(txhash));
  if (queryTaskErr) {
    if (queryTaskErr instanceof zkgapi__namespace.Error.TDNoTaskFound) {
  ...
  const result = await zkgapi__namespace.waitProve(zkWasmProviderUrl, taskId, true);
...
async function upload(options) {
  ...
  const { isUploadSuccess, response, errorMessage } = await zkgapi.upload(
    mappingPath,
    wasmPath,
    yamlPath,
    // userAddress,
...
async function verify(options) {
  ...
  const proofParams = await zkgapi.getVerifyProofParamsByTaskID(taskId, zkWasmProviderUrl);
  const verifyResult = await zkgapi.verify(
    ...
    if (error instanceof zkgapi.Error.ProveTaskNotReady)
...
async function publish(options) {
  ...
  const provider = new ethres.ethers.providers.JsonRpcProvider(JsonRpcProviderUrl);
  const signer = new ethres.ethers.Wallet(userPrivateKey, provider);
  const wasm = fs__default.readFileSync(wasmPath);
  const wasmUint8Array = new Uint8Array(wasm);
  const [err, publishTxHash] = await to__default(zkgapi__namespace.publish(
  ...
  if (err) {
    if (err instanceof zkgapi__namespace.Error.GraphAlreadyExist) {
...
async function loadConfigFromFile(configFile, configRoot = process.cwd()) {
  ...
  const loader = await unconfig.createConfigLoader({
    cwd: configRoot,
    sources: [configFile ? {
      files: configFile || "",
      extensions: [],
...
function create(targetDir, template) {
  createZkgraph.init(targetDir, template);
}
...
async function deposit(options) {
  ...
  const provider = new ethres.ethers.providers.JsonRpcProvider(jsonRpcUrl);
  const signer = new ethres.ethers.Wallet(userPrivateKey, provider);
  const transactionHash = zkgapi__namespace.deposit(
    provider,
    signer,
...
async function run() {
  try {
    const cli = cac__default("zkgraph");
