import { ByteArray, Bytes } from "../common/types/bytes";
import { BigInt } from "../common/type";
import { uint8ArrayToUint32Array } from "../utils/conversion";
...
export function testBigInt(): void {
  ...
  // fromBytesLittleEndian, fromBytes (BigEndian)
  bigint = BigInt.fromBytesLittleEndian(
    ByteArray.fromI32LittleEndian(I32.MAX_VALUE),
  );
  assert(
    bigint.toString() == I32.MAX_VALUE.toString(),
    "BigInt fromBytesLittleEndian (ByteArray) failed",
  );
  bigint = BigInt.fromBytesLittleEndian(
    ByteArray.fromI32LittleEndian(I32.MIN_VALUE),
    true,
  );
  assert(
    bigint.toString() == I32.MIN_VALUE.toString(),
    "BigInt fromBytesLittleEndian (ByteArray negative) failed",
  );
  bigint = BigInt.fromBytesLittleEndian(
    Bytes.fromU32LittleEndian(U32.MAX_VALUE),
  );
  assert(
    bigint.toString() == U32.MAX_VALUE.toString(),
    "BigInt fromBytesLittleEndian (Bytes) failed",
  );
  bigint = BigInt.fromBytes(Bytes.fromU32LittleEndian(U32.MAX_VALUE));
  assert(
    bigint.toString() == U32.MAX_VALUE.toString(),
    "BigInt fromBytes (Bytes negative) failed",
  );
  bigint = BigInt.fromBytesLittleEndian(Bytes.fromU32LittleEndian(0));
  assert(bigint.toString() == "0", "BigInt fromBytesLittleEndian zero failed");
  bigint = BigInt.fromBytes(Bytes.fromU32LittleEndian(0));
  assert(bigint.toString() == "0", "BigInt fromBytes zero failed");
